...
@Component({
  ...
  template: `
    @if (loading()) {
      <div class="text-center text-gray-500 text-3xl mt-4 mb-2">Loading today's matches...</div>
    } @else if (error()) {
      <div class="text-center text-red-500 text-3xl mt-4 mb-2">{{ error() }}</div>
    } @else {
      <div class="text-center text-gray-500 text-3xl mt-4 mb-2">Last updated: {{ lastUpdated() }}</div>
      @if (competitions().size === 0) {
        <div class="text-center text-gray-500 text-3xl mt-4">No games scheduled for today</div>
      } @else {
