...
@Component({
  ...
  template: `
    <div class="px-3 py-[10px] sm:px-4 sm:py-3 border-b-2 border-gray-200 last:border-b-0 transition-colors duration-200 hover:bg-gray-50">
      <div class="text-sm font-bold text-gray-800 mb-1 leading-[1.4]">
      ...
      <div class="mt-0.5">
        <div class="text-[13px] text-gray-500 mb-1">{{ match().stadium_name }}, {{ match().stadium_city }}, {{ match().stadium_country }}</div>
        @if (match().neutral_venue) {
          <div class="text-xs text-gray-500 font-medium mt-0.5">
            <span class="text-amber-600 font-semibold text-xs">Neutral Venue</span>
          </div>
        }
      </div>
    </div>
