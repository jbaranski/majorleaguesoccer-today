...
@Component({
  ...
  template: `
    <div class="px-3 py-[10px] sm:px-4 sm:py-3 border-b-2 border-gray-200 last:border-b-0 transition-colors duration-200 hover:bg-gray-50">
      <div class="text-xl font-bold text-gray-800 mb-1 leading-[1.4]">
        <strong>{{ match().home_team_name }} vs {{ match().away_team_name }}</strong>
      </div>
      <div class="text-base font-medium text-gray-600 mb-1">{{ formatTime(match().planned_kickoff_time) }}</div>
      <div class="mt-0.5">
        <div class="text-base text-gray-500 mb-1">{{ match().stadium_name }}, {{ match().stadium_city }}, {{ match().stadium_country }}</div>
