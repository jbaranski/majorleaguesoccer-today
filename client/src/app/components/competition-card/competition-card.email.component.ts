...
@Component({
  ...
  template: `
    <div style="border: 2px solid #e5e7eb; margin-bottom: 16px; font-family: ${FONT};">
      <div [style.background-color]="isResult() ? '#f0fdf4' : '#ede9fe'" style="padding: 10px 12px 6px; border-bottom: 2px solid #e5e7eb;">
        <div style="font-size: 18px; font-weight: bold; text-transform: uppercase; letter-spacing: 0.5px; color: #1f2937; line-height: 1.2; margin-bottom: 4px;">{{ competition() }}</div>
        <div style="font-size: 16px; font-style: italic; color: #4b5563; margin-bottom: 4px;">{{ formatDate() }}</div>
